...
use_cache=0
if [ -f $HOME/.config/waypaper/wallpaper_cache ] ;then
    use_cache=1
fi
...
# ----------------------------------------------------
# Copy wallpaper to cache file
# -----------------------------------------------------

magick $wallpaper $image_cache_file
...
if [ -f $HOME/.config/waypaper/wallpaper-effect.sh ] ;then
    effect=$(cat $HOME/.config/waypaper/wallpaper-effect.sh)
    if [ ! "$effect" == "off" ] ;then
        ...
        if [ -f $generated_versions/$effect-$wallpaper_filename ] && [ "$force_generate" == "0" ] && [ "$use_cache" == "1" ] ;then
            ...
            source $HOME/.config/waypaper/wallpaper/$effect
        fi
...
echo ":: Update Hyprpanel Wallpaper with $used_wallpaer"
hyprpanel sw $used_wallpaper
...
killall -e hyprpaper & 
sleep 1; 
wal_tpl=$(cat $HOME/.config/waypaper/hyprpaper.tpl)
